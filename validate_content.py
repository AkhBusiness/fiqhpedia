...
# كل ما ليس سيريلياً ولا ترقيماً داخل كلمة سيريلية. كان النطاق محصوراً في
# اللاتينية الأساسية، ففاتته اللاتينية الموسّعة: `ікáма` مرّت بحرف `á`
# (U+00E1). والقاعدة الآن بالاستثناء لا بالتعداد، فلا يفوتها حرف جديد.
LATIN_LOOKALIKE = re.compile(
    r"[^\u0400-\u04FF\s\d«»…\-\u2013\u2014.,;:!?()\[\]'\u2019\u02bc\u02bf\u02be*_`/\ufdfa\u0301]"
)
